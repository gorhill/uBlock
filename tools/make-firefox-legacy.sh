...
echo "*** AdNauseam.firefox-legacy: Copying files"
...
echo "*** AdNauseam.firefox-legacy: concatenating content scripts"
...
echo "*** AdNauseam.firefox-legacy: Generating meta..."
python tools/make-firefox-meta.py $DES/ "$2"
printf "*** AdNauseam.firefox-legacy: Generating web accessible resources...\n"
cp -R src/web_accessible_resources $DES/
python3 tools/import-war.py $DES/


if [ "$1" = all ]; then
    echo "*** AdNauseam.firefox-legacy: Creating package..."
    pushd $(dirname $DES/) > /dev/null
    zip artifacts/adnauseam.firefox.xpi -qr *
    popd > /dev/null
fi

echo "*** AdNauseam.firefox-legacy: Package done."
echo
